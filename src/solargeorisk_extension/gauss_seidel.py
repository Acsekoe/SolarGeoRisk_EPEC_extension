...
import solargeorisk_extension.model_intertemporal as _it
...
# =============================================================================
# Intertemporal Gauss-Seidel solver
# =============================================================================
def solve_gs_intertemporal(
    ...
    *,
    ...
    """Gauss-Seidel solver for the 4-period intertemporal EPEC model.

    Theta dicts are keyed by (region, time) for scalar strategies and by
    (imp/exp, region, time) for tariffs.
    """
    if iters < 1:
        raise ValueError("iters must be >= 1")
    if not (0.0 < omega <= 1.0):
    ...
    kcap_2025 = data.Kcap_2025 if data.Kcap_2025 is not None else {r: float(data.Qcap[r]) for r in data.regions}
    ...
    if initial_state:
        ...
        theta_tau_imp: Dict[Tuple[str, str, str], float] = {
            (im, ex, tp): float(initial_state.get("tau_imp", {}).get((im, ex, tp), 0.0))
            for im in data.regions for ex in data.regions for tp in times
        }
        theta_tau_exp: Dict[Tuple[str, str, str], float] = {
            (ex, im, tp): float(initial_state.get("tau_exp", {}).get((ex, im, tp), 0.0))
            for ex in data.regions for im in data.regions for tp in times
        }
        theta_s: Dict[Tuple[str, str], float] = {
            (r, tp): float(initial_state.get("s", {}).get((r, tp), 0.0))
            for r in data.players for tp in times
        }
        theta_Icap: Dict[Tuple[str, str], float] = {
            (r, tp): float(initial_state.get("Icap", {}).get((r, tp), 0.0))
            for r in data.players for tp in times
        }
        theta_Dcap: Dict[Tuple[str, str], float] = {
            (r, tp): float(initial_state.get("Dcap", {}).get((r, tp), 0.0))
            for r in data.players for tp in times
        }
    else:
        theta_Q = {(r, tp): 0.8 * float(kcap_2025[r]) for r in data.players for tp in times}
        theta_tau_imp = {(im, ex, tp): 0.0 for im in data.regions for ex in data.regions for tp in times}
        theta_tau_exp = {(ex, im, tp): 0.0 for ex in data.regions for im in data.regions for tp in times}
        theta_s = {(r, tp): 0.0 for r in data.players for tp in times}
        theta_Icap = {(r, tp): 0.0 for r in data.players for tp in times}
        theta_Dcap = {(r, tp): 0.0 for r in data.players for tp in times}
    ...
    def _ti_scale(im: str, ex: str) -> float:
        return max(float(data.tau_imp_ub[(im, ex)]), 1e-3)

    def _te_scale(ex: str, im: str) -> float:
        return max(float(data.tau_exp_ub[(ex, im)]), 1e-3)
    ...
    def _update_prox_reference() -> None:
        ...
        for im in data.regions:
            for ex in data.regions:
                for tp in times:
                    if im == ex:
                        ti_last[im, ex, tp] = 0.0
                    else:
                        ti_last[im, ex, tp] = float(theta_tau_imp.get((im, ex, tp), 0.0))
        for ex in data.regions:
            for im in data.regions:
                for tp in times:
                    if ex == im:
                        te_last[ex, im, tp] = 0.0
                    else:
                        te_last[ex, im, tp] = float(theta_tau_exp.get((ex, im, tp), 0.0))
    ...
    for it in range(1, iters + 1):
        ...
        for p in sweep_order:
            ...
            te_sol = state.get("tau_exp", {})
            s_sol = state.get("s", {})
            Icap_sol = state.get("Icap", {})
            Dcap_sol = state.get("Dcap", {})
            obj_sol = state.get("obj", {})
            ...
            # Update subsidy + capacity decisions
            for tp in times:
                sk = (p, tp)
                if sk in s_sol:
                    theta_s[sk] = (1.0 - omega) * theta_s[sk] + omega * float(s_sol[sk])
                if sk in Icap_sol:
                    theta_Icap[sk] = (1.0 - omega) * theta_Icap[sk] + omega * float(Icap_sol[sk])
                if sk in Dcap_sol:
                    theta_Dcap[sk] = (1.0 - omega) * theta_Dcap[sk] + omega * float(Dcap_sol[sk])
        ...
        for im in data.regions:
            for ex in data.regions:
                ...
                for tp in times:
                    key = (im, ex, tp)
                    r_strat = max(r_strat, _scaled_change(theta_tau_imp[key], prev_ti[key], _ti_scale(im, ex)))
        ...
        for ex in data.regions:
            for im in data.regions:
                ...
                for tp in times:
                    key = (ex, im, tp)
                    r_strat = max(r_strat, _scaled_change(theta_tau_exp[key], prev_te[key], _te_scale(ex, im)))
